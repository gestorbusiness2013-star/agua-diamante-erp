...
import { Calendar as CalendarIcon, MapPinned, LocateFixed, TrendingUp, DollarSign, Users, ShoppingBag, Activity } from 'lucide-react';
...
export default function VendedoresPage() {
    ...
    const sellerStats = useMemo(() => {
        ...
        return sellers.map(seller => {
            ...
            return {
                ...seller,
                totalSold: sSales.reduce((sum, s) => sum + (Number(s?.totalAmount) || 0), 0),
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                ...
                <Card><CardHeader><CardTitle>Rendimiento</CardTitle></CardHeader>
                                ...
                                {sellerStats.map(s => (
                                    <TableRow key={s.id}>
                                        <TableCell className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col"><span className="text-sm font-medium">{s.name || 'Usuario'}</span><span className="text-[10px] text-muted-foreground">{s.saleCount} pedidos</span></div>
                                        </TableCell>
                                        <TableCell className="text-right text-emerald-600 font-bold">{formatCurrency(s.totalCommission)}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
